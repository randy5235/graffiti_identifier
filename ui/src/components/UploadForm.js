...
class UploadForm extends React.Component{
  ...
  /*global componentDidMount EXIF:true*/ // stop eslint from throwing errors/warnings since ol depends on the following external script
  componentDidMount(){
    const script = document.createElement("script");
    script.src = "https://cdn.jsdelivr.net/npm/exif-js"; // For retrieving Exif data
    script.async = true;
  ...
  ValidateForm(event)
  {
    ...
    function GPSDegToDecString( lat, latRef, lon, lonRef)
    {
      ...
      if (files[0] !== undefined) // file selected
      {
        if (files[0].size > 8*1024*1024) // 8 MB size limit
        {
          console.log('file too large');
          return false;
        }
        else if  (locInput.value.length > 0) // location manually entered
        {
          ...
          EXIF.getData(files[0], function()
        {
          ...
          else // set the location input (Should confirm the formatting)
          {
    ...
    return (
      ...
      onSubmit={this.ValidateForm}>
        <div>
          ...
          <input id= "upload-location" name="location" type="hidden" placeholder="no exif, gps coordinates please"/>
